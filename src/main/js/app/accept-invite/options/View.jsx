...
const t = scoped('nti-login.accept-invite.login.View', {
	...
	inviteInstructions: {
		canCreateAccount: 'You\'ve been invited to %(brandName)s. Please login or create an account to continue.',
		cannotCreateAccount: 'You\'ve been invited to %(brandName)s. Please login to your account to continue.'
	},
	setupInstructions: ''
});
...
function getInstructions (isAccountSetup, handshake, brandName) {
	if (!handshake) { return ''; }
	if (isAccountSetup) { return t('setupInstructions'); }

	return canCreateAccount(handshake) ? t('inviteInstructions.canCreateAccount', {brandName}) : t('inviteInstructions.cannotCreateAccount', {brandName});
}
...
function AcceptInviteOptionsPage ({isAccountSetup, handshake, invitation, setup, hasPing, error:storeError, busy, forcePassword}) {
	...
	return (
		<Page.Content>
			<Page.Header />
			<Page.Body>
				<Page.Description description={getInstructions(isAccountSetup, handshake, brandName)}/>
				<PaddedContainer>
